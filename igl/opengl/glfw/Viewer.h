		...
		namespace glfw
		{
			...
			public:
			   ...
			   // enum class MouseMode { None, Rotation, Zoom, Pan, Translation} mouse_mode;
				...
				void initLinkAxes();
				double tipToDest();
				void print_rotation_mats();
				void print_tip_position();
				...
				Eigen::Matrix3d get_euler_mat(Eigen::Vector3d angles);

				//number of links
				int linksNum;
				
				//destination position (sphere center location).
				Eigen::Vector4d destPos;


				//joint position
				std::vector<Eigen::Vector4d> tipPositions;
